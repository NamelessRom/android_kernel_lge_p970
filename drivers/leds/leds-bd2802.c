...
const char *BRIGHTNESS_MODE_str[] = {
...
enum KEY_LED_REG {
	KEY_LED_REG_CURRENT1 = 0,
	KEY_LED_REG_CURRENT2 = 1,
	KEY_LED_REG_WAVEPATTERN = 2,
};
...
static inline u8 bd2802_get_reg_addr(enum KEY_LED led, enum KEY_LED_REG led_reg)
{
	...
	u8 current1, u8 current2, u8 wave)
{
	...
	reg = bd2802_get_reg_addr(led, KEY_LED_REG_CURRENT1);
	bd2802_write_byte(bd2802_led->client, reg, current1);
	reg = bd2802_get_reg_addr(led, KEY_LED_REG_CURRENT2);
	bd2802_write_byte(bd2802_led->client, reg, current2);
	reg = bd2802_get_reg_addr(led, KEY_LED_REG_WAVEPATTERN);
...
static void bd2802_locked_dyn_on(struct bd2802_led *bd2802_led)
{
	if (bd2802_led->onoff == ONOFF_DYN_OFF) {
		gpio_set_value(RGB_KEY_LED_GPIO, 1);
		udelay(200);
...
static void bd2802_locked_dyn_off(struct bd2802_led *bd2802_led)
{
	if (bd2802_led->onoff == ONOFF_DYN_ON) {
		...
		gpio_set_value(RGB_KEY_LED_GPIO, 0);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (led = KEY_LED_MIN; led <= KEY_LED_MAX; ++led) {
		...
		if (value0 > 100 || value1 > 100) {
			err("Brightness for key %s too high\n", KEY_LED_str[led]);
			return -EINVAL;
		}
...
static int update_brightness_mode(struct kobj_props *p, int new_val)
{
	...
	if (bd2802_led->brightness_mode != new_val &&
		bd2802_led->onoff == ONOFF_DYN_ON) {

		auto_brightness_listener_off(bd2802_led);
		bd2802_led->brightness_mode = new_val;
		auto_brightness_listener_on(bd2802_led);
	} else {
		bd2802_led->brightness_mode = new_val;
	}
...
static void set_brightness_to(struct bd2802_led *bd2802_led, int delay_ms,
		int new_brightness)
{
	const struct pattern *active_pattern =
		bd2802_led->saved_patterns[bd2802_led->active_input];
	...
	if (is_blinking[active_pattern->type]) {
		info("Setting brightness to %d for %s\n", new_brightness,
			PATTERN_str[active_pattern->type]);
		fade_stop(&bd2802_led->fade);
		bd2802_led->brightness = new_brightness;
		bd2802_update_active_pattern(bd2802_led);
	} else {
		info("Fading brightness to %d for %s after %d ms\n", delay_ms,
			PATTERN_str[active_pattern->type], delay_ms);
		fade_brightness_delayed(&bd2802_led->fade, delay_ms,
			bd2802_led->brightness, new_brightness);
	}
}
...
static int update_brightness(struct kobj_props *p, int new_val)
{
	...
	set_brightness_to(bd2802_led, 0, new_val);
...
static int update_brightness_levels(struct kobj_props *p, int *new_vals, size_t size)
{
	...
	if (size == 1 || size == 2) {
		int max = size == 1 ? ALS_LEVELS : new_vals[2];

		util_fill_exp(p->val, ALS_LEVELS, bd2802_led->fade.ld_offset,
			new_vals[0], max);
...
static DEV_INT_ATTR(brightness, 0644, 0, BRIGHTNESS_MAX,
...
static DEV_INFO_ATTR(info, 0444,
	...
	"\n"
	"Write 1 to button to illuminate all the buttons, write 0 to cancel.\n"
	"Write 0 or 1 to touchkey_enabled to enable or disable the touchkey input");

static DEV_ATTR(pattern, 0200, NULL, pattern_store);

static DEV_BOOL_ATTR(button, 0644, I2C_OFFSET(button), update_button);

static DEV_ENUM_SHORT_ATTR(brightness_mode, 0644, BRIGHTNESS_MODE,
		...
		unsigned long als_level, void *data)
{
	...
	set_brightness_to(bd2802_led, bd2802_led->adapt_brightness_delay,
		bd2802_led->brightness_levels[als_level]);
