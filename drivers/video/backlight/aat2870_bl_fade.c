...
#define fade_dbg(...) dev_dbg(fade->dev, __VA_ARGS__)
#define fade_info(...) dev_info(fade->dev, __VA_ARGS__)
#define fade_warn(...) dev_warn(fade->dev, __VA_ARGS__)
#define fade_err(...) dev_err(fade->dev, __VA_ARGS__)
...
static uint32_t find_next_brightness(struct fade *fade)
{
	...
	fade_dbg("Fade: next %d ldo %d lim %d\n", next, ld[next + o], ld_next_o);
...
static void fade_brightness_schedule(struct fade *fade)
{
	...
	if (fade->enabled) {
		...
		fade_dbg("Fade step from %u to %u scheduled in %u ms\n",
...
static void set_brightness(struct work_struct *ws)
{
	...
	fade->set_brightness_to(fade, fade->brightness_next);
...
void fade_stop_internal(struct fade *fade)
{
	if (fade->state != FADE_STATE_STOPPED) {
		fade_info("Stopping fade\n");
	...
	uint32_t from, uint32_t to)
{
	...
	if (fade->state != FADE_STATE_STOPPED) {
		...
		if (direction == new_direction) {
			...
			fade_dbg("Setting new target brightness of running fade to %u\n",
	...
	fade_info("%s brightness from %d to %d\n",
	...
	fade_info("%s from %u to %u after %u ms (interval: %u ms)\n",
...
static bool *bool_from_kobj_offset(struct kobject *kobj, long *data)
{
	return (bool *)(((char *)kobj) + *data);
}

#define KOBJ_OFFSET(_val) int_from_kobj_offset, \
	(offsetof(struct fade, _val) - offsetof(struct fade, kobj))
#define KOBJ_BOOL_OFFSET(_val) bool_from_kobj_offset, \
	(offsetof(struct fade, _val) - offsetof(struct fade, kobj))

static KOBJ_INFO_ATTR(info, 0444,
	"Support for kernel based fading when changing the brightness\n\n"
	...
	"The fade interval is configured in fade_full_ival\n"
	"The minimum interval for one fade step is configured in fade_min_step_ival");
static KOBJ_BOOL_ATTR(enabled, 0644, KOBJ_BOOL_OFFSET(enabled), NULL);
static KOBJ_INT_ATTR(full_ival, 0644, 50, 10000, KOBJ_OFFSET(full_ival_ms), NULL);
...
static KOBJ_INT_ATTR(min_step_ival, 0644, 5, 100,
	KOBJ_OFFSET(min_step_ival_ms), NULL);

static struct attribute *attrs[] = {
	&kobj_info_attr_info.attr.kobj_attr.attr,
	&kobj_bool_attr_enabled.attr.kobj_attr.attr,
	&kobj_int_attr_full_ival.attr.kobj_attr.attr,
	&kobj_int_attr_full_ival.min_attr.kobj_attr.attr,
	&kobj_int_attr_full_ival.max_attr.kobj_attr.attr,
	&kobj_int_attr_ld_offset.attr.kobj_attr.attr,
	&kobj_int_attr_ld_offset.min_attr.kobj_attr.attr,
	&kobj_int_attr_ld_offset.max_attr.kobj_attr.attr,
	&kobj_int_attr_min_step_ival.attr.kobj_attr.attr,
	&kobj_int_attr_min_step_ival.min_attr.kobj_attr.attr,
	&kobj_int_attr_min_step_ival.max_attr.kobj_attr.attr,
	NULL,
};
...
{
	int ret = kobject_init_and_add(&fade->kobj, &kt, &dev->kobj, "fade");
