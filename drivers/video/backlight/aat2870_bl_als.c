...
#define dbg(...) dev_dbg(&als->als_props->client->dev, __VA_ARGS__)
#define info(...) dev_info(&als->als_props->client->dev, __VA_ARGS__)
#define warn(...) dev_warn(&als->als_props->client->dev, __VA_ARGS__)
#define err(...) dev_err(&als->als_props->client->dev, __VA_ARGS__)
...
static ssize_t last_measured_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	struct als *als = container_of(kobj, struct als, kobj);
	uint32_t msecs_ago = jiffies_to_msecs(jiffies - als->level_jif);
...
static KOBJ_INFO_ATTR(info, 0444,
	...
	"\n"
	"The automatic polling interval can be set via poll_ival in ms,\n"
...
static KOBJ_ATTR(last_measured, 0444, last_measured_show, NULL);

static KOBJ_INT_ATTR(poll_ival, 0644,
	ALS_IVAL_MIN, ALS_IVAL_MAX,
	KOBJ_OFFSET(struct als, poll_ival_ms), NULL);
static KOBJ_INT_MIN_MAX_ATTR(poll_ival, 0444);

static KOBJ_ATTR(trigger_measure, 0200, NULL, trigger_measure_store);

static struct attribute *attrs[] = {
	&kobj_val_attr_listeners.kobj_attr.attr,
	&kobj_val_attr_info.kobj_attr.attr,
	&kobj_val_attr_gain_mode.kobj_attr.attr,
	&kobj_val_attr_gain_resistor.kobj_attr.attr,
	&kobj_val_attr_gain_percentage.kobj_attr.attr,
	&kobj_attr_last_measured.attr,
	&kobj_attr_level.attr,
	&kobj_val_attr_poll_ival.kobj_attr.attr,
	&kobj_val_attr_poll_ival_min_max.kobj_attr.attr,
...
static void update_als_level_stage1(struct work_struct *ws)
{
	...
	if (als->measure_running) {
		dbg("als measure already running, just rescheduling\n");
		goto reschedule;
	}
...
reschedule:
	if (als->listeners)
		schedule_delayed_work(&als->update_als_level_stage1,
			msecs_to_jiffies(als->poll_ival_ms));
	else
		dbg("Not scheduling a next als measure as there are no listeners\n");
...
int als_add_listener(struct als *als, struct notifier_block *listener)
{
	...
	if (res != 0) {
		warn("srcu_notifier_chain_register returned %d\n", res);
		goto out;
	}
	...
	if (als->listeners++ == 0)
		...
		schedule_delayed_work(&als->update_als_level_stage2, 0);
...
void als_set_poll_ival(struct als *als, int poll_ival_ms)
{
	mutex_lock(&als->mutex);

	als->poll_ival_ms = poll_ival_ms;
	cancel_delayed_work(&als->update_als_level_stage1);
	schedule_delayed_work(&als->update_als_level_stage1, 0);
